...
function hasAnimations(code: string): boolean {
  return /animation|transition|keyframe|whileHover|whileTap|whileInView|initial|animate|exit/.test(
    code
  );
}
...
export function componentSelfReview(
  ...
  code: string,
  designTokens: Record<string, string | Buffer> = {}
): FrontendSelfReview {
  const issues: string[] = [];
  ...
  if (hasAnimations(code) && !usesReducedMotion(code)) {
    issues.push(
      `${componentName}: Has animations but missing useReducedMotion (accessibility)`
    );
  }
  ...
  if (designTokens['design-tokens.json']) {
    try {
      ...
      if (hardcodedColors) {
        ...
        const invalidColors = hardcodedColors.filter(
          (c) => !validColors.includes(c.toLowerCase())
        );

        if (invalidColors.length > 0) {
          issues.push(
            `${componentName}: Hardcoded colors not in design tokens: ${invalidColors.join(
              ', '
            )}`
          );
        }
 ...
 */
export function getFrontendExecutor(
  ...
  return {
    ...
    async generateArtifacts(
      ...
      if (!llmClient) {
        throw new Error(
          'llmClient is required in context for getFrontendExecutor.generateArtifacts'
        );
      }
...
1. **src/app/page.tsx** - Landing page with distinctive aesthetic
2. **src/app/globals.css** - CSS with custom fonts, OKLCH colors, and design tokens

And implement these UI components from the component inventory (using shadcn/ui patterns and design tokens):
${componentTypes
  .map(
    (type, i) =>
      `${
        i + 3
      }. **src/components/ui/${type}.tsx** - Distinctive ${type} component with custom fonts and motion`
  )
  .join('\n  ')}
...
Return ALL components as separate code blocks with proper filename markers.`
...
- Distinctive, non-generic layout patterns`;
      ...
      const artifacts: Record<string, string | Buffer> = parseFrontendArtifacts(
        llmResponse.content,
        phase
      );
    ...
    validateArtifacts(
      ...
      const forbiddenPatterns = [
        ...
        { pattern: 'arial', message: 'Forbidden font: Arial detected' },
        {
          ...
          message: 'Forbidden pattern: purple-gradient detected',
        },
        {
          pattern: 'linear-gradient(to right, #8b5cf6',
          message: 'Forbidden pattern: purple gradient detected',
        },
        {
          pattern: 'blob background',
          message: 'Forbidden pattern: blob background detected',
        },
        {
          pattern: 'blob',
          message:
            'Suspicious pattern: blob detected (may indicate generic AI design)',
        },
        {
          pattern: 'font-family: inter',
          message: 'Forbidden font-family: Inter detected',
        },
        {
          pattern: 'font-family: roboto',
          message: 'Forbidden font-family: Roboto detected',
        },
        {
          pattern: "font-family: 'inter'",
          message: 'Forbidden font-family: Inter detected',
        ...
        {
          pattern: 'centered content card',
          message: 'Generic pattern: centered content card detected',
        },
        {
          pattern: 'hero section',
          message: 'Generic pattern: hero section detected',
        },
        {
          pattern: 'gradient text',
          message:
            'Suspicious pattern: gradient text (may indicate generic AI)',
        },
      ];
      ...
      const requiredPatterns = [
        ...
        {
          pattern: '@import',
          message: 'Missing custom @import font declaration',
        },
        {
        ...
        { pattern: 'var(--', message: 'Missing CSS custom properties' },
        {
          ...
          message: 'Missing distinctive display font (Space Grotesk)',
        },
        {
          pattern: 'outfit',
          message: 'Missing distinctive display font (Outfit)',
        },
        { pattern: 'syne', message: 'Missing distinctive display font (Syne)' },
        {
          pattern: 'fraunces',
          message: 'Missing distinctive display font (Fraunces)',
        },
        {
          pattern: 'playfair display',
          message: 'Missing distinctive display font (Playfair Display)',
        },
        { pattern: 'dm sans', message: 'Missing refined body font (DM Sans)' },
        {
          pattern: 'public sans',
          message: 'Missing refined body font (Public Sans)',
        },
      ...
      if (strictMode) {
        ...
        const hasCustomFont = requiredPatterns
          .slice(4, 12)
          .some((p) => allContent.includes(p.pattern));
        if (!hasCustomFont) {
          results.push({
            severity: 'error',
            artifactId: 'frontend-components',
            message:
              'Missing distinctive custom font (Space Grotesk, Syne, Outfit, Fraunces, or similar)',
          });
        }
      ...
      const genericPatterns = [
        {
          regex: /className=".*flex.*center.*"/i,
          message: 'Generic flex-center pattern detected',
        },
        {
          regex: /className=".*w-full.*h-full.*"/i,
          message: 'Generic full-width/height pattern detected',
        },
        {
          regex: /import.*from\s+['"]react['"]/i,
          message: 'Basic React import (should have motion)',
        },
      ];
...
function parseFrontendArtifacts(
  content: string,
  phase: string
  ...
  const codeBlockRegex =
    /```(?:tsx|typescript|ts|javascript|js|css)?[ \t]*\n?filename:\s*([^\n]+)\n([\s\S]*?)```/g;
  let match;
