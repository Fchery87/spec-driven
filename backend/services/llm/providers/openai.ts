    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          logger.error('OpenAI API call timed out', new Error('Request timeout'), {
        ...
        if (attempt === retries) {
          logger.error('OpenAI API call failed after all retries', error instanceof Error ? error : new Error(String(error)), { attempt, phase });
          throw error;
        }

        logger.warn(`OpenAI API error, retrying (${attempt}/${retries})...`, {
