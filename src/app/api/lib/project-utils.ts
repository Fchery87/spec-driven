...
export const getProjectMetadata = async (slug: string, ownerId?: string) => {
    ...
    try {
      ...
      if (ownerId) {
        ...
        if (!metadata.created_by_id) {
          logger.warn(
            'Metadata missing owner in R2 payload, falling back to database',
            { slug, ownerId }
          );
          ...
          logger.warn('Metadata ownership mismatch from R2', {
            slug,
            ownerId,
            createdBy: metadata.created_by_id,
          });
          return null;
        } else {
      ...
      logger.debug(
        'Failed to get project metadata from R2, trying local file system',
        { slug }
      );
    }
  }
  ...
  try {
    ...
    if (existsSync(path)) {
      ...
      if (ownerId) {
        ...
        if (!metadata.created_by_id) {
          logger.warn(
            'Metadata missing owner in filesystem payload, falling back to database',
            { slug, ownerId }
          );
          // Don't return here - let it fall through to database fallback
        } else if (metadata.created_by_id !== ownerId) {
          // Only fail if owner exists but doesn't match
          logger.warn('Metadata ownership mismatch from filesystem', {
            slug,
            ownerId,
            createdBy: metadata.created_by_id,
          });
  ...
  try {
    const { ProjectDBService } = await import(
      '@/backend/services/database/drizzle_project_db_service'
    );
    ...
    if (project) {
      ...
      if (project.clarificationState) {
        try {
          clarificationState = JSON.parse(
            project.clarificationState
          ) as ClarificationState;
        } catch {
          logger.warn('Failed to parse clarification_state from database', {
            slug,
          });
        }
      }
    ...
    logger.debug('Failed to load project metadata from database', {
      slug,
...
export const saveProjectMetadata = async (
  ...
  if (isR2Configured()) {
    try {
      ...
      logger.debug(
        'Failed to save project metadata to R2, trying local file system',
        { slug }
      );
    }
  }
...
export const listArtifacts = async (slug: string, phase: string) => {
  logger.info('listArtifacts called', {
    slug,
    phase,
  ...
  if (isR2Configured()) {
    try {
      ...
      logger.info('Artifacts listed from R2', {
        slug,
        phase,
      ...
      logger.warn(
        'Failed to list artifacts from R2, trying database fallback',
        {
          slug,
          phase,
  ...
  try {
    ...
    if (project) {
      logger.info('Project found in database, fetching artifacts', {
        slug,
        phase,
        projectId: project.id,
      });
      const dbArtifacts = await dbService.getArtifactsByPhase(
        project.id,
        phase
      );
      logger.info('Database artifacts query result', {
        slug,
        phase,
        count: dbArtifacts.length,
      });

      if (dbArtifacts.length > 0) {
        logger.info('Artifacts listed from database', {
          slug,
          phase,
          count: dbArtifacts.length,
        });
        for (const artifact of dbArtifacts as Array<{
          filename: string;
          content: string;
        }>) {
    ...
    logger.error(
      'Failed to list artifacts from database',
  ...
  if (existsSync(phasePath)) {
    try {
      ...
      logger.info('Artifacts listed from local filesystem', {
        slug,
        phase,
        count: names.length,
      });
...
export const writeArtifact = async (
  ...
  if (isR2Configured()) {
    try {
      ...
      const err =
        r2Error instanceof Error ? r2Error : new Error(String(r2Error));
      ...
      logger.warn('Falling back to local filesystem for artifact', {
        slug,
        phase,
        name,
      });
    ...
    logger.debug('R2 not configured, using local filesystem for artifacts', {
      slug,
      phase,
      name,
    });
  ...
  try {
    ...
    logger.debug('Artifact saved to local filesystem', {
      slug,
      phase,
      name,
      path,
    ...
    logger.error('Failed to save artifact to both R2 and filesystem', err, {
      slug,
      phase,
      name,
    });
...
export const readArtifact = async (
  ...
  if (isR2Configured()) {
    try {
      ...
      // If it's a ZIP or other binary, return Buffer
      if (
        name.endsWith('.zip') ||
        name.endsWith('.png') ||
        name.endsWith('.jpg')
      ...
      const err =
        r2Error instanceof Error ? r2Error : new Error(String(r2Error));
      logger.debug(
        'Failed to read artifact from R2, trying local file system',
        {
          slug,
          phase,
          name,
    ...
    logger.debug('R2 not configured, trying local filesystem', {
      slug,
      phase,
      name,
    });
  ...
  try {
    ...
    logger.debug('Artifact read from local filesystem', {
      slug,
      phase,
      name,
      path,
  ...
  try {
    ...
    const latest = dbArtifacts.find(
      (a: { filename: string }) => a.filename === name
    );
    if (!latest || !latest.content) {
      throw new Error('Artifact not found in database');
    }

    // Database content is currently stored as text (likely base64 or similar if binary)
    // For now, we assume text for database results unless we update the schema
    return latest.content;
  } catch (dbError) {
    const err = dbError instanceof Error ? dbError : new Error(String(dbError));
    logger.error(`Error reading artifact from all sources`, err, {
      slug,
      phase,
      name,
    });
...
export const deleteProject = (slug: string): boolean => {
  try {
    ...
    if (existsSync(projectPath)) {
      ...
      logger.info('Project directory deleted from local filesystem', {
        slug,
        projectPath,
      });
    } else {
      logger.debug(
        'Project directory does not exist locally, nothing to delete',
        { slug, projectPath }
      );
    }
    return true;
...
export async function persistProjectToDB(
  ...
  try {
    ...
    if (!ownerId) {
      logger.warn('Skipping database persistence because owner is missing', {
        slug,
      });
      return;
    }
