  ...
  async (
    ...
    try {
      ...
      const artifactsByPhase: Record<
        string,
        Array<{ name: string; size: number; content: string | null }>
      > = {};

      for (const phase of allPhases) {
        const phaseArtifacts = await listArtifacts(slug, phase);
        logger.info(`Phase ${phase} artifacts from listArtifacts`, {
          slug,
          phase,
          count: phaseArtifacts.length,
        });

        // If artifacts exist for this phase, read their content from R2 or filesystem
        const artifactsWithContent = await Promise.all(
          phaseArtifacts.map(
            async (artifact: { name: string; size: number }) => {
              try {
                const rawContent = await readArtifact(
                  slug,
                  phase,
                  artifact.name
                );
                const content = asString(rawContent);
                logger.info(`Read artifact content`, {
                  slug,
                  phase,
                  name: artifact.name,
                ...
                logger.error(
                  `Error reading artifact ${artifact.name} from phase ${phase}:`,
        ...
        if (artifactsWithContent.length > 0) {
          logger.info(`Including phase ${phase} in response`, {
            slug,
            phase,
      ...
      logger.info('Final artifacts response', {
        slug,
        phases: Object.keys(artifactsByPhase),
        totalArtifacts: Object.values(artifactsByPhase).flat().length,
      });
