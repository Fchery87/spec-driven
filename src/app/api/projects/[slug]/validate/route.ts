    ...
    try {
      ...
      if (dbProject) {
        ...
        await dbService.recordPhaseHistory(
          dbProject.id,
          'VALIDATE',
          summary.overallStatus === 'fail' ? 'failed' : 'completed'
        );
      }
...
async function checkRequirementToTaskMapping(
  ...
  const items = prdReqs.map((req) => ({
    ...
    message: taskReqs.includes(req)
      ? 'Mapped to task'
      : 'No implementing task found',
  }));
  ...
  return {
    ...
    details: `${prdReqs.length - unmapped.length}/${
      prdReqs.length
...
async function checkApiToDataModelMapping(
  ...
  const entities = [...dataModel.matchAll(entityPattern)].map((m) =>
    m[1].toLowerCase()
  );
  ...
  const unmapped = apiEntities.filter(
    (e) => !entities.includes(e) && !['error', 'pagination', 'meta'].includes(e)
  );
  const items = apiEntities.map((entity) => ({
    item: entity,
    status:
      entities.includes(entity) ||
      ['error', 'pagination', 'meta'].includes(entity)
        ? ('pass' as const)
        : ('warning' as const),
    message: entities.includes(entity)
      ? 'Found in data model'
      : 'Not in data model',
  }));
  ...
  return {
    ...
    details: `${apiEntities.length - unmapped.length}/${
      apiEntities.length
    } schemas mapped`,
    items,
  };
...
async function checkPersonaConsistency(
  ...
  const definedPersonas = [...personas.matchAll(personaPattern)].map((m) =>
  ...
  const items = definedPersonas.map((persona) => ({
    item: persona,
    status: prdPersonaRefs.includes(persona)
      ? ('pass' as const)
      : ('warning' as const),
    message: prdPersonaRefs.includes(persona)
      ? 'Referenced in PRD'
      : 'Not referenced in PRD',
  }));
  ...
  return {
    ...
    details: `${definedPersonas.length - unreferenced}/${
      definedPersonas.length
    } personas referenced in PRD`,
    items,
  };
}
  ...
  for (const tech of techKeywords) {
    ...
    if (inDecision) {
      items.push({
        ...
        message: inArch
          ? 'Consistent'
          : 'In stack-decision but not in architecture',
      });
    }
  }
...
async function checkEpicTaskConsistency(
  ...
  const items = taskEpics.map((epic) => ({
    ...
    message: definedEpics.includes(epic)
      ? 'Defined in epics.md'
      : 'Not defined in epics.md',
  }));
  ...
  return {
    ...
    details: `${taskEpics.length - orphaned.length}/${
      taskEpics.length
    } epics valid`,
    items,
  };
...
async function checkUnresolvedClarifications(
  ...
  const resolvedPattern = /\[RESOLVED:[^\]]+\]/g;
  const items: {
    item: string;
    status: 'pass' | 'fail' | 'warning';
    message?: string;
  ...
  if (resolvedCount > 0) resolutionInfo.push(`${resolvedCount} user-resolved`);
  if (assumedCount > 0) resolutionInfo.push(`${assumedCount} AI-assumed`);
  const resolutionSuffix =
    resolutionInfo.length > 0 ? ` (${resolutionInfo.join(', ')})` : '';
  ...
  return {
    ...
    details:
      items.length === 0
        ? `All clarifications resolved${resolutionSuffix}`
        : `${items.length} unresolved clarifications found. Return to ANALYSIS phase and complete the Clarification step.`,
...
async function checkAIAssumptionsDocumented(
  ...
  return {
    ...
    details:
      items.length === 0
        ? 'No AI assumptions made'
        : `${items.length} AI assumptions documented`,
    items: items.length > 0 ? items : undefined,
  };
}
...
async function checkDesignSystemCompliance(
  ...
  items.push({
    ...
    message: hasPurplePrimary
      ? 'Purple/indigo detected as primary'
      : 'Compliant',
  });
...
async function checkTestFirstCompliance(
  ...
  const hasTestSection =
    /test.*specification|test.*criteria|acceptance.*test/i.test(tasks);
  ...
  items.push({
    ...
    message:
      hasTestSection || hasTestFirst
        ? 'Test sections found'
        : 'No explicit test sections',
  });
  ...
  items.push({
    ...
    message: hasGherkin
      ? 'Gherkin format found'
      : 'Consider adding Given/When/Then',
  });
...
async function checkConstitutionalCompliance(
  ...
  const hasModularStructure = /module|component|service|library/i.test(
    architecture
  );
  items.push({
    item: 'Article 1: Library-First',
    status: hasModularStructure ? 'pass' : 'warning',
    message: hasModularStructure
      ? 'Modular structure evident'
      : 'Consider modular boundaries',
  });
  ...
  const serviceCount = (architecture.match(/service|microservice/gi) || [])
    .length;
  items.push({
  ...
  const hasIntegrationTests =
    /integration.*test|real.*database|real.*service/i.test(tasks);
  items.push({
...
`;
  ...
  for (const phase of phases) {
    ...
    for (const artifact of phaseArtifacts) {
      matrix += `| ${phase} | ${artifact.name} | ${(
        artifact.size / 1024
      ).toFixed(1)}KB | ✅ Present |\n`;
    }
  }
