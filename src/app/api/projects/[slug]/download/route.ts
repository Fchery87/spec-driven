...
- \`SPEC_PM/\` - Product Requirements Document (PRD)
- \`SPEC_ARCHITECT/\` - Data model and API specifications
- \`SPEC_DESIGN_TOKENS/\` - Design tokens and styling guidelines
- \`SPEC_DESIGN_COMPONENTS/\` - Component specifications and user journey maps
- \`FRONTEND_BUILD/\` - Frontend components and code
- \`DEPENDENCIES/\` - Project dependencies and requirements
- \`SOLUTIONING/\` - Architecture, epics, and task breakdown
- \`VALIDATE/\` - Validation reports and coverage matrix
- \`AUTO_REMEDY/\` - Automated remediation reports
